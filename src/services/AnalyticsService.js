...
class AnalyticsService {
  async analysisOrderInYear(branchId = undefined, orderStatus = undefined) {
    return Promise.all(
      listMonths.map(async (month, index) => {
  ...
  async analysisTotalRevenueInYear(isActual, branchId = undefined) {
    return Promise.all(
      listMonths.map(async (month, index) => {
        const startDate = new Date(new Date().getFullYear(), index, 1)
        const endDate = new Date(new Date().getFullYear(), index + 1, 0)
