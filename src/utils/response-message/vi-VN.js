const MapResponseMessage = {
  ...
  exists: (dataName, extraString = '') =>
    `${dataName} đã tồn tại ${extraString}`,
  ...
  hasRelatedData: (dataName) =>
    `Bạn không thể xóa ${dataName} do có liên quan đến các dữ liệu khác`,
  // blockSoftDeleteCategory: `Không thể xóa danh mục vì còn `,
  notInSoftDeletionState: (action) =>
    action === 'recover'
      ? `Không thể khôi phục vì không ở trạng thái đã xóa`
      : `Không thể thực hiện hành động xóa vĩnh viễn vì không ở trạng thái đã xóa`,
  alreadyDeleted: (dataName) =>
    `${dataName} đã ở trạng thái xóa, bạn không thể thực hiện thao tác này lần nữa`,
  hasNotCompletedRequest: (dataName) =>
    `Không thể xóa ${dataName} vì còn yêu cầu nhập hàng chưa hoàn thành`,
